/**
 * @file base_auto_fix.c
 * @brief Base Station Auto-Fix 모드 구현
 */
...
// 설정
#define AVERAGING_DURATION_SEC 60     // 평균 계산 시간 (60초)
#define MAX_SAMPLES 600               // 최대 샘플 수 (10Hz * 60초)
#define MIN_SAMPLES 30                // 최소 샘플 수
 ...
 * @brief GGA 데이터 업데이트
 */
void base_auto_fix_on_gga_update(const gps_gga_t *gga) {
  if (state != BASE_AUTO_FIX_AVERAGING) {
    return;
  }

  // Fix가 RTK Fix인지 확인
  if (gga->fix != GPS_FIX_RTK_FIX) {
    return;
  }

  // 샘플 저장
  if (sample_count < MAX_SAMPLES) {
    samples[sample_count].lat = gga->lat;
    samples[sample_count].lon = gga->lon;
    samples[sample_count].alt = gga->alt;
    sample_count++;

    // 10개마다 로그 (10Hz 기준 1초)
    if (sample_count % 10 == 0) {
      LOG_INFO("샘플 수집: %lu/%d (%.2f%%)",
               sample_count, MAX_SAMPLES,
               (sample_count * 100.0) / MAX_SAMPLES);
    }
  }
}
...
static bool switch_to_base_fixed_mode(void) {
  ...
  // Unicore UM982: MODE BASE 명령
  LOG_INFO("UM982 Base Fixed 모드 설정");

  // 좌표를 십진수 형식으로 변환
  char cmd[128];
  snprintf(cmd, sizeof(cmd), "MODE BASE %.9f %.9f %.3f",
